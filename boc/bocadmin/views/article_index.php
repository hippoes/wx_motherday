		...
		<table class="table table-striped table-hover select-list">
			<thead>
				<tr>
					...
					<th>排序</th>
					<th>标题</th>
                    <?php if (in_array($cid,$need_type)): ?><th>所属分类</th><?php endif; ?>
					<th>时间</th>
			...
			<tbody class="sort-list">
				<?php foreach ($list as $v):?>
					<tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
                        ...
                        <?php if (in_array($cid,$need_type)): ?><td><?php echo colty_name($v['ctype']) ?></td><?php endif; ?>
						<td> <?php echo date("Y/m/d H:i:s",$v['timeline']); ?> </td>
						<td>
							<div class="btn-group">
								<?php if (!in_array($cid,$no_flag)): ?><?php include 'inc_ui_flag.php'; ?><?php endif; ?>
                                <?php if ($cid==48): ?>
                                    <?php if ($v['flag1']): ?>
                                        <a href="#"  title="取消头条推荐"  class='btn btn-primary btn-small btn-ajax-flag1' data-id="<?php echo $v['id'] ?>" data-flag1="0">  <i class="fa fa-flag"></i> </a>
                                    <?php else: ?>
                                        <a href="#" title="头条推荐" class='btn btn-small btn-ajax-flag1' data-id="<?php echo $v['id'] ?>" data-flag1="1"> <i class="fa fa-flag"></i> </a>
                                    <?php endif ?>
...
<script>
	require(['adminer/js/ui'],function(ui){
		var article = {
            ...
            ,url_flag1: "<?php echo site_urlc('article/flag1'); ?>"
    ...
    ui.btn_flag1(article.url_flag1);		// 推荐
