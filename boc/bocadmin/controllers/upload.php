...
class Upload extends Base_Controller {
	...
	public function index(){
		...
		//缩略图大小限制
		$smallsize=$this->input->get('smallsize', TRUE);
		if(!empty($smallsize)){
			$tmp_smallsize = explode('x',$smallsize);
				if (is_int(intval($tmp_smallsize[0])) && is_int(intval($tmp_smallsize[1])) ) {
					$config['image_versions']['thumbnail'] = array(
						'crop' => true,
						'max_width' => $tmp_smallsize[0],
						'max_height' => $tmp_smallsize[1]
					);
				}
		}
	...
	public function downloads(){
		...
		if($fids = $this->input->get('fids',TRUE)){
			$where = array();
			foreach (explode(',', $fids) as $value) {
				if (is_numeric($value)) {
					array_push($where, $value);
			...
			foreach ($f as $k => $v) {
				$this->zip->read_file(UPLOAD_PATH.$v['url']);
			}
			$this->zip->archive( APPPATH.'cache/'.$filename.'.zip');
			$this->zip->download($filename.'.zip');
		}else{
			$this->output->set_content_type('application/json')->set_output(json_encode(array("status"=>0,"msg"=>"没有正确的参数")));
		}
