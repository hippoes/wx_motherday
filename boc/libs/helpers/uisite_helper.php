...
function download_files($file_addr,$file_name){
	if (file_exists($file_addr)){
		...
		header("Expires: Thu, 12 Apr 2017 14:00:00 GMT");
		...
		function header_seoinfo($cid,$ctype)
		{
			...
			if($CI->router->class == 'welcome' and $CI->router->method == "index"){
				...
				if($ctype){
					...
					if(!empty($ctypeinfo)){
						$title=$ctypeinfo['title'].'-'.$CI->mcfg->get_config('site','title_suffix');
					}else{
						$title=$CI->mcfg->get_config('site','title_seo').'-'.$CI->mcfg->get_config('site','title_suffix');
					}
				}else{
					if(!empty($columnsinfo['title_seo'])){
						$title=$columnsinfo['title_seo'].'-'.$CI->mcfg->get_config('site','title_suffix');
					}else{
						$title=$CI->mcfg->get_config('site','title_seo').'-'.$CI->mcfg->get_config('site','title_suffix');
					}
				}
...
// Banner
if(!function_exists('bannerinfo'))
{
	function bannerinfo($cid)
	{
		$CI=&get_instance();
		$CI->load->database();
		$CI->load->model('banners_model','mbanners');
		$bannerinfo = $CI->mbanners->get_one(array('cid'=>$cid,'audit'=>1),'id,title,content,timeline,photo,entitle');
		return $bannerinfo;
	}
}

// foot_product_types
// if(!function_exists('foot_product_types'))
// {
// 	function foot_product_types()
// 	{
// 		$CI=&get_instance();
// 		$CI->load->database();
// 		$CI->load->model('coltypes_model','mtype');
// 		$foot_product_types = $CI->mtype->get_all(array('fid'=>0,'cid'=>28),'id,title',array('sort_id'=>'asc'));
// 		return $foot_product_types;
// 	}
// }
